...
void Query::Query1(const Table& customer_table, const Table& zonecost_table) {
    ...
    Column<bool> customer_column_active = std::get<Column<bool>>(customer_table.get_table().find("ACTIVE")->second);
    idx1 = 0;
    ...
    const Column<int> customer_column_zone = std::get<Column<int>>(customer_table.get_table().find("ZONE")->second);
    const Column<int> zonecost_column_zoneid = std::get<Column<int>>(zonecost_table.get_table().find("ZONEID")->second);
    const Column<std::string> zonecost_column_zonedesc = std::get<Column<std::string>>(zonecost_table.get_table().find("ZONEDESC")->second);
    idx1 = 0;
    ...
    //output
    idx1 = 0;
    const Column<std::string> customer_column_lname = std::get<Column<std::string>>(customer_table.get_table().find("LNAME")->second);
...
void Query::Query2(const Table& lineitem_table, const Table& products_table) {
    ...
    const Column<int> products_column_barcode = std::get<Column<int>>(products_table.get_table().find("BARCODE")->second);
    idx1 = 0;
    for (Data<int> products_data_barcode : products_column_barcode.get_data()) {
        if (products_data_barcode.raw.has_value()) {
            Column<int> lineitem_column_barcode = std::get<Column<int>>(lineitem_table.get_table().find("BARCODE")->second);
            Column<std::string> lineitem_column_uname = std::get<Column<std::string>>(lineitem_table.get_table().find("UNAME")->second);
    ...
    //output
    idx1 = 0;
    Column<std::string> products_column_proddesc = std::get<Column<std::string>>(products_table.get_table().find("PRODDESC")->second);
    for (bool a1 : answer1) {
        if (a1 == true) {
